...
Hitable* RandomScene2(Camera& cam, int nx, int ny) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        glm::vec3 random = RandomOnHorizontalUnitCircle();
        random *= Random() * 1 + 2.5f;
        if (chooseMat < 0.8f) { // diffuse
            list->push_back(new Sphere(random, 0.2f,
                new Lambertian(glm::vec3(Random() * Random(), Random() * Random(), Random() * Random()))));
        }
        else if (chooseMat < 0.95f) { // metal
            list->push_back(new Sphere(random, 0.2f,
                new Metal(glm::vec3(0.5f * (1 + Random()), 0.5f * (1 + Random()), 0.5f * (1 + Random())), 0.5f * (1 + Random()))));
        }
        else { // glass
